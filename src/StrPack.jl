...
import Base: read, write, isequal
...
function extract_annotations(exprIn)
    ...
    asize = :(Dict{Symbol,Integer}(zip([$(fieldnames...)], [$(asizes...)])))
...
@compat unpack{T}(str::Union{AbstractString, Array{UInt8,1}}, ::Type{T}) = unpack(IOBuffer(str), T)
...
align_native = align_table(align_default, let
    for var in (:i8a, :i16a, :i32a, :i64a, :f32a, :f64a)
        @eval $var = Array(UInt, 1)
    end
